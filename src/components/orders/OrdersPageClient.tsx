...
// Простой прогресс для карточки (визуально)
...
export default function OrdersPageClient() {
	const [tab, setTab] = useState<OrdersTab>('available');
	const [search, setSearch] = useState('');
	const [urgentOnly, setUrgentOnly] = useState(false);
	const [isPageVisible, setIsPageVisible] = useState(true);

	// Видимость вкладки: чтобы polling не долбил в фоне
	useEffect(() => {
	...
	const query = useMemo(() => {
		// Фильтры "Срочные" и т.п. применяем только к available (PENDING),
		// чтобы не усложнять UX на active/history.
		return {
			scope: tab,
			search: search.trim() || undefined,
			urgentOnly: tab === 'available' ? urgentOnly : undefined,
			limit: 50,
		};
	}, [tab, search, urgentOnly]);

	const refetchInterval = useCallback(() => {
		if (!isPageVisible) return false;
		// polling только там, где он реально нужен
		if (tab === 'active') return 2500;
		if (tab === 'available') return 5000;
		return false; // history — без polling
	}, [tab, isPageVisible]);

	const { data, isLoading, isFetching, error, refetch } = useQuery({
		queryKey: ['orders', tab, query],
		queryFn: () => getAvailableOrders(query),
		staleTime: tab === 'active' ? 2_000 : tab === 'available' ? 5_000 : 30_000,
		gcTime: 5 * 60 * 1000,
	...
	const headerTitle = useMemo(() => {
		// чтобы не было "История заявок" всегда
		return tab === 'available' ? 'Доступные заявки' : tab === 'active' ? 'Активные заявки' : 'История заявок';
	}, [tab]);
